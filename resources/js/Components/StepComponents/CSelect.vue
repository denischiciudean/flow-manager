...
export default {
    ...
    methods: {
        search() {
            ...
            this.$emit('changed', [this.data_key, this.search_term]);
            if (!term) {
                this.results = [];
            }
            if (term && this.options) {
                this.results = this.options.filter(it => it.toLowerCase().includes(term.toLowerCase()))
            }
        },
        modifiedInput() {
            // console.log(this.selected)
            this.$emit('changed', [this.data_key, this.selected]);
            this.search_term = this.selected
            this.results = [];
        }
    }
